<div class="mt-5">
    <div class="border border-dotted border-gray-400 rounded-lg p-4">
        <button id="toggleButton_edit_role" class="flex items-center w-full text-left focus:outline-none">
            <div class="flex items-center justify-center">
                <span class="text-gray-600 mr-2 text-xl">✍️</span>
                <span class="text-gray-700 text-base">Add your editor role</span>
            </div>
        </button>

        <div id="content_edit_role" class="mt-4 max-h-0 overflow-hidden transition-all duration-500 ease-in-out">
            <div class="p-4 border border-solid border-gray-200 rounded-lg">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-lg font-bold">Editor Role</h2>
                    <button x-data @click="$dispatch('show-editor-role-modal')" class="text-blue-600 hover:text-blue-800">
                ...
                @if (is_null($editorRoles->role) && is_null($editorRoles->journal))
                    <p class="text-gray-500 mt-2">Please fill your information</p>
                @else
                    <div class="grid grid-cols-6 mt-2 ">
                        <p class="text-black font-bold col-span-1">Role: </p>
                        <p class="text-gray-700 col-span-5">{{ $editorRoles->role }}</p>
                    </div>
                    <div class="grid grid-cols-6 mt-2 ">
                        <p class="text-black font-bold col-span-1">Journal: </p>
                        <p class="text-gray-700 col-span-5">{{ $editorRoles->journal }}</p>
                    </div>
                @endif
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const toggleButtons = document.getElementById('toggleButton_edit_role');
        const content_introduction = document.getElementById('content_edit_role');
